{ lib, config, pkgs, ... }:
{
  options.my.vesktop.smol = lib.mkEnableOption ''
    Hide channel and member list until hover
  '';

  config = {
    home-manager.users."${config.my.username}" = {
      home.packages = with pkgs; [ vesktop ];
      # @TODO: this shid don work cause for some reason a theme can't import stylesheets?
      xdg.configFile."vesktop/themes/colornix.theme.css".text = with config.lib.stylix.colors; ''
        /**
        * @name colornix
        * @author Colorman
        * @version 0.0.0
        * @description Preferred discord Nix theme
        **/
        ...
        ${if config.my.vesktop.smol then ''
