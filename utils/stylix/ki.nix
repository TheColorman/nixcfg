  ...
  flake.nixosModules.utils-stylix-ki =
    ...
    in
    {
      config = lib.mkIf kiEnabled {
        home-manager.users = {
          "${username}" = {
            xdg.configFile."ki/themes/stylix.json".source = base16Theme;

            programs.ki.settings.theme = lib.mkForce "Base16 Stylix";
          };

          # Special case - I use Ki as root, and the config requires the theme
          "root".xdg.configFile."ki/themes/stylix.json".source = base16Theme;
        };
      };
    };
}
