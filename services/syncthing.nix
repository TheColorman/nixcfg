...
}:
{
  flake.nixosModules.services-syncthing =
    ...
    let
      ...
      inherit ((import "${inputs.nix-secrets}/evaluation-secrets.nix").syncthing)
        devices
        ;
      ...
      otherHostFolders =
        ...
        |> filterAttrs (host: _: host != config.networking.hostName)
        |> mapAttrs (_: configuration: configuration.config.my.syncthing.folders or { })
        |> mapAttrs (
          _: module:
          module.config.my.syncthing.folders or { } |> filterAttrs (_folder: opts: opts != null) |> attrNames
        )
        |> mapAttrsToList (
          host:
          map (folder: {
      ...
      syncthingSops = "services/syncthing/${config.networking.hostName}";
    in
    {
      ...
      config = {
        ...
        systemd = {
          ...
          services.syncthing = {
            ...
            requires = optionals cfg.enableMounts (
              activeFolders
              |> mapAttrsToList (
                _folder: opts: escapeSystemdPath "${config.services.syncthing.dataDir}/folders/${opts.id}.mount"
              )
